...
public partial class admin_AfterService_AfterService : System.Web.UI.Page
{
    ...
    private void Bind()
    {
        ...
        string filedOrder = "puchaseDate desc";
        DataTable dt = new NetWise.DataAccess.AfterSalesService().GetListJoinEx(0, strWhere, filedOrder).Tables[0];
    ...
    protected void rptMsg_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        ...
        {
            if (new NetWise.DataAccess.AfterSalesService().Delete(Convert.ToInt32(e.CommandArgument)))
            {
                NetWise.DBUtility.JsControl.Show(this, "删除记录成功!", "AfterService.aspx");
            }
        }
    }
    ...
    protected void btnDelAll_Click(object sender, EventArgs e)
    {
        if ("1" == ddlSelect.SelectedValue && rptMsg.Items.Count > 0)
        {
            ...
            for (int i = 0; i < rptMsg.Items.Count; i++)
            {
                ...
                {
                    new NetWise.DataAccess.AfterSalesService().Delete(Convert.ToInt32(hfID.Value));
                    needRefresh = true;
                }
            }
            if (needRefresh)
            {
                NetWise.DBUtility.JsControl.Show(this, "删除成功！", "AfterService.aspx");
            }
            else
            {
