...
public partial class admin_AfterService_AfterServiceInfo : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (null != Request["m_Id"] && string.Empty != Request["m_Id"].ToString())
            {
                ...
                {
                    var ds = new NetWise.DataAccess.AfterSalesService().GetModelEx(m_Id).Tables[0];
                    if (ds.Rows.Count>0)
                    {
                        cName.Value = ds.Rows[0]["V1"].ToString(); //中文国家
                        puchaseDate.Text = Convert.ToDateTime(ds.Rows[0]["puchaseDate"].ToString()).ToString("yyyy-MM-dd HH:mm:ss");
                        pName.Value = ds.Rows[0]["pName"].ToString();
                        city.Text = ds.Rows[0]["city"].ToString();
                        phoneNumber.Value = ds.Rows[0]["phoneNumber"].ToString();
                        orderNumber.Value = ds.Rows[0]["orderNumber"].ToString();
                        productType.Value = ds.Rows[0]["productType"].ToString();
                        productDescription.Value = ds.Rows[0]["productDescription"].ToString();
                        serialNumber.Text = ds.Rows[0]["serialNumber"].ToString();
                        versionNumber.Text = ds.Rows[0]["versionNumber"].ToString();

                        //英文
                        cEname.Value = ds.Rows[0]["EN_country"].ToString();
                        pEname.Value = ds.Rows[0]["pEname"].ToString();
                        EN_city.Text = ds.Rows[0]["EN_city"].ToString();
                        EN_orderNumber.Value = ds.Rows[0]["EN_orderNumber"].ToString();
                        EN_productType.Value = ds.Rows[0]["EN_productType"].ToString();
                        EN_productDescription.Value = ds.Rows[0]["EN_productDescription"].ToString();
                        EN_serialNumber.Text = ds.Rows[0]["EN_serialNumber"].ToString();
                        EN_versionNumber.Text = ds.Rows[0]["EN_versionNumber"].ToString();
                    }
                }
            }
        }
