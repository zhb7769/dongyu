...
namespace NetWise.DataAccess
{
    ...
    public partial class Parts
    {
        ...
        public int GetMaxID()
        {
            return DbHelperSQL.GetMaxID("P_Id", "Parts");
        }
        /// <summary>
        /// 增加一条数据
        /// </summary>
        public int Add(NetWise.Entity.Parts model)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(NetWise.Entity.Parts model)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        /// 得到一个对象实体
        /// </summary>
        public NetWise.Entity.Parts GetModel(int P_Id)
        {

            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 P_Id,CN_Name,EN_Name,CN_Label,EN_Label,S_Img,M_Img,O_Img,L_Img,CN_Synopsis,EN_Synopsis,ClassId,IsPage,IsTop,CreateTime,Sort,Remark from Parts ");
            strSql.Append(" where P_Id=@P_Id");
            SqlParameter[] parameters = {
                    new SqlParameter("@P_Id", SqlDbType.Int,4)
            };
        ...
        /// 得到一个对象实体
        /// </summary>
        public NetWise.Entity.Parts DataRowToModel(DataRow row)
        {
            NetWise.Entity.Parts model = new NetWise.Entity.Parts();
            if (row != null)
            {
        ...
        /// <param name="ID"></param>
        /// <returns></returns>
        public bool UpdatePartsForSort(int Sort, int P_Id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update Parts set ");
            strSql.Append("Sort=@Sort ");
        ...
        public DataSet GetListByClassId(int ClassId)
        {
            ...
            //                            )
            //                          SELECT * FROM Parts AS p WHERE ClassId IN (SELECT T.ID FROM T ) ORDER BY p.CreateTime DESC";

            string sql = @"SELECT * FROM Parts a,f_PartsNum(@ID) b WHERE a.ClassId = b.ID ";
        ...
        {
            StringBuilder strSQL = new StringBuilder();
            strSQL.Append("SELECT * FROM Parts a,f_PartsNum(@ID) b WHERE a.ClassId = b.ID ");
        ...
        {
            int ret = 0;
            string sql = "select CN_ReadCount from Parts where P_Id = '" + P_Id + "'";
        ...
        {
            string sql = "update Parts set CN_ReadCount='" + CN_ReadCount + "' where P_Id= '" + P_Id + "'";
        ...
        {
            int ret = 0;
            string sql = "select EN_ReadCount from Parts where P_Id = '" + P_Id + "'";
            object obj = NetWise.DBUtility.DbHelperSQL.GetSingle(sql);
            if (null != obj && "" != obj.ToString())
            {
                ret = Convert.ToInt32(obj);
        ...
        {
            string sql = "update Parts set EN_ReadCount='" + EN_ReadCount + "' where P_Id = '" + P_Id + "'";
            return NetWise.DBUtility.DbHelperSQL.ExecuteSql(sql);
        }
        ...
        public DataSet GetListByPageJoinPic(string strWhere, string orderby, int startIndex, int endIndex)
        {
            ...
            strSql.Append(")AS Row, T.*,(select top 1 pro_img from pro_att_info t2 where T.P_Id=t2.pro_id order by at_id asc) pro_img  from Parts T ");
            if (!string.IsNullOrEmpty(strWhere.Trim()))
            {
                strSql.Append(" WHERE " + strWhere);
            }
