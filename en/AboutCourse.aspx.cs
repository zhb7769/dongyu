using System;
using System.Data;
using System.Web.UI;
using NetWise.DataAccess;

public partial class EnAboutCourse : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        HengTuo.PageValidation.Sqlzy(this);
        if (!IsPostBack)
        {
            BindTitle();
            BindCourse();
        }
    }

    private void BindTitle()
    {
        NetWise.DataAccess.ArticleClass dal = new NetWise.DataAccess.ArticleClass();
        NetWise.Entity.ArticleClass model = dal.GetModel(3);
        if (model != null)
        {
            ltBanTitle.Text = model.EN_Name;
            ltTitle.Text = model.EN_Name;
        }
    }

    private void BindCourse()
    {
        NetWise.DataAccess.Article dal = new NetWise.DataAccess.Article();
        DataSet ds = dal.GetList(0, " and ArticleClassId=3", "Sort asc");
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
        {
            rptCourse.DataSource = ds;
            rptCourse.DataBind();
        }
    }
}
